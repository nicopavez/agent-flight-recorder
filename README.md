...
worked — so when something breaks (and in production, it does), there's
a real trace to look at instead of a shrug.
...
Three data points, not just a hunch:
...
The recording side lives in Workato (a Data Table + two API-triggered
recipes — see [`docs/BUILD_PLAN.md`](docs/BUILD_PLAN.md)). The *reading*
side — turning a trace into something a human can actually use —
is [`trace_viewer.py`](trace_viewer.py), which works standalone against
...
narrative summary instead. Deliberately didn't:
